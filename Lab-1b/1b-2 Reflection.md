...
This lab had me create new users on my linux system and to do user group management along with assigning file permissions. In the below images, I added alice, bob and mallory by using the `adduser` command and I verified it, as shown in the first image, by using `less /etc/passwd` to my virtual machine and made a group using the `groupadd` command assigned only alice and bob to a shared group, verified using the `less /etc/group` command.
...
Next, I made a directory using `mkdir`, and changed who owned it using the `chown` command to change the owner to alice, the `chgrp` command to chang ethe group to the sharedgroup. Then I made a list of files using `touch` and directed it to the group folder by adding `/home/shared` where I also learned a shortcut to add multiple files using `{1..10}` instead of manually adding them one by one.
...
The image below verifies which users have what permissions. I was able to switch users by using the `su` command to switch to Alice to view using `cat` and to write using `echo` onto a text file, then switched using `su` to bob, where he cannot use `echo` to write over as he does not have the permissions, and switching to mallory using `su` and showing that mallory outright has no access to it at all using `cat`.
...
After that I learned about the `-r` flag to edit the permissions of all the files in the `/home/shared` folder, using `chmod`, `chgrp` and `chown` along with `-r` to change permissions to all the items inside.
...
The lab outlined how to add mallory as part of the sudo group by using `usermod -aG sudo` to add mallory to the sudo group. The following image shows that mallory is allowed to run all commands by running `sudo -l`
...
Finally, I had to remove the file using `rm` and `-r` to remove everything, I did this and afterwards went into alice using `su` and checked that it was not there at all.
